...
| A5-001 | P1 | Hero board caption | `Hero.tsx:486-497` | “Live” pill + “Try the live board” → `/demo` implies interactive live CRM. Destination is static product moments; also fights pre-launch honesty. Residual play icon on secondary CTA still cues video after A3-001 Watch→See. | Fixed |
| A5-002 | P1 | Hero switch strip vs FAQ | `Hero.tsx:385-387` vs `marketing-launch.ts` HOME_FAQ | Same overclaim as **A2-001**: “free migration” + “full history come with you” vs FAQ hands-on / no one-click / white-glove later. Narrative break in the homepage argument. Copy already aligned to priority cutover help (present in Hero since A3 Hero land). | Fixed |
...
| A5-008 | P2 | Tone | Home spine | Mostly honest-operator (Q4 2026, no card, reserve ≠ access). Hype pockets were A5-001/002; cleared with Preview/walkthrough + cutover honesty. | Fixed |
