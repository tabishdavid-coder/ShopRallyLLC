...
import { importTireStockCsv } from "@/server/actions/tire-stock";

export function ImportTiresDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  function handleFile(file: File) {
    ...
    reader.onload = () => {
      ...
      startTransition(async () => {
        const result = await importTireStockCsv(text);
        if (!result.ok) {
        ...
        const partial =
          "rowErrors" in result && result.rowErrors?.length
            ...
            : "";
        setSuccess(`Imported ${result.created} tire(s). Skipped ${result.skipped}.${partial}`);
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4 text-sm">
          <p className="text-muted-foreground">
            Download the template, fill in your tire stock rows, then upload. Stock numbers must be
            unique per shop.
          </p>
          <Button variant="outline" size="sm" className="gap-1.5" asChild>
            <a href="/api/tires/template">
