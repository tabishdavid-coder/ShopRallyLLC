...
import { adjustTireQuantity } from "@/server/actions/tire-stock";
...
export function AdjustTireQuantityDialog({
  tireId,
  stockNumber,
  currentQty,
  open,
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await adjustTireQuantity({ tireId, delta, reason: finalReason });
      if (!result.ok) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Stock <span className="font-medium text-foreground">{stockNumber}</span> — current QOH{" "}
